/**
 * ta-coin.js -- PORTED FROM STILL LIFE, `flying-dungeon/src/coin.js`.
 *
 ...
 *
 * ⚠️ IT WAS A LITERAL COPY BAR THE CLASS NAME, and that is worth keeping true:
 ...
 *
 * RENAMED `Coin` -> `TaCoin` because this game already has `Flies` (the
 ...
 */
  /* ⚠️ ASSET KEYS COME FROM CONFIG, AND THAT IS A COLLISION FIX, NOT TIDYING.
     The beat 'em up ALREADY has a `fly` key -- its scenery swarm, `FLIES.SHEET`,
     drawn by src/flies.js -- so loading Still Life's mosca under that name would
     silently repaint the background flies of every level with a different
     insect. Every key this class reads is therefore nameable; the defaults are
     Still Life's own, so that game is untouched.
     ⚠️ `boom` IS DELIBERATELY SHARED AND NOT PREFIXED: the beat 'em up's
     `BOOM_SHEET` is the very same `v2:flying-dungeon/saborosa-boom.webp` with
     byte-identical `BOOM_RECTS` (checked), and this game has a history of
     running out of VRAM on old cards -- a second copy of one atlas for
     tidiness is exactly the wrong trade. See PERFORMANCE.md. */
/**
...
class TaCoin {
  ...
  render(ctx, camX, camY, worldW) {
    ...
    const sheet = this.assets.getDrawable((this.cfg.keyCoin || 'coin_') + this.variant);
    if (!sheet) return;
  ...
  _blitHitFx(ctx, camX, camY, worldW) {
    ...
    if (!fx) return;
    const sheet = this.assets.getDrawable(this.cfg.keyFly || 'fly');
    if (!sheet) return;
    const i = Math.min(c.coinHitFxFrames - 1, Math.floor(fx.t / c.coinHitFxMs));
    const r = c.FLY_RECTS[1 + i];               // 0 is the live fly; 1..4 burst
    if (!r) return;
    const s = (c.coinSizePx * c.coinHitFxSize) / TaCoin._widest(c.FLY_RECTS.slice(1));
  ...
  _blitBoom(ctx, camX, camY, worldW) {
    ...
    if (!r) return;
    const sheet = this.assets.getDrawable(this.cfg.keyBoom || 'boom');
    if (!sheet) return;
    const s = (c.coinSizePx * c.coinBoomSize) / TaCoin._widest(rects);
