 ...
 *
 * ⚠️ READ THIS BEFORE JUDGING A BOX'S POSITION BY EYE.
 *
 ...
 *
 * ⚠️ EVERY BOX DRAWN HERE COMES FROM THE SAME FUNCTION THE RESOLVER CALLS.
 ...
 *
 * ⚠️ Z IS NOT SCREEN Y. On the belt everything lives at `beltTopY + z`. Drawing
...
class Debug {
  ...
  render(ctx, all, stage, backdrop, camX) {
    ...
    const planBottom = this._plan(ctx, live, stage, camX);
  ...
  _sideBoxes(ctx, all, camX) {
    for (const f of all) {
      ...
      ctx.strokeStyle = canHit ? DBG.body : DBG.iframe;
      if (!canHit) ctx.setLineDash([3, 3]);
   ...
   * in the resolver.
   */
  _plan(ctx, all, stage, camX) {
    const b = stage.bounds();
    const PX = 22, PY = 112, PW = 430, HEAD = 15;

    /* ⚠️ THE SCALE IS ANCHORED TO THE CAMERA'S VIEW, NOT TO stage.bounds().
    ...
    for (const f of all) {
      ...
      ctx.strokeStyle = f.vulnerable() ? DBG.body : DBG.iframe;
      ctx.setLineDash(f.vulnerable() ? [] : [2, 2]);
      ctx.strokeRect(px(f.x) - hw, pz(f.z) - hz, hw * 2, hz * 2);
      ctx.setLineDash([]);
      // Altitude, which the plan cannot show — printed instead of implied.
      if (f.jumpY > 1) {
        ctx.fillStyle = DBG.dim;
        ctx.fillText('y' + Math.round(f.jumpY), px(f.x) + hw + 2, pz(f.z) - 5);
      }
    }
