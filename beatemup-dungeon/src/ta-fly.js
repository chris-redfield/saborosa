/**
 * ta-fly.js -- PORTED FROM STILL LIFE, `flying-dungeon/src/fly.js`.
 *
 ...
 *
 * RENAMED `Fly` -> `TaFly` because this game already has `Flies` (the
...
class TaFly {
  ...
  render(ctx, camX, camY, worldW) {
    ...
    if (this.state === 'landed') {
      const dead = this.assets.getDrawable(this.cfg.keyFlyDead || 'flyDead');
      if (dead) blit(dead, c.FLY_DEAD_RECT, this.restX, this.restY - camY, this.restRot);
      return;
    }

    if (this.state === 'alive') {
      const img = this.assets.getDrawable(this.cfg.keyFly || 'fly');
      if (img) {
